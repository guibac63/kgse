...
class ContactCrudController extends AbstractCrudController
{
    ...
    public function validationForm($entityInstance):void
    {
        ...
        $contactBirthDate=$entityInstance->getBirthDate();

        if ($entityInstance->getCountry()!==null){
            $contactCountry = $entityInstance->getCountry()->getId();
        }
        ...
        if(!isset($contactCountry)){
            $this->session->getFlashbag('')->add('danger','Veuillez renseigner une nationalité pour le contact');
            $this->validator = false;
            //if one of the target has the same nationality of one of the agents
        }elseif ($contactBirthDate>new \DateTime('now')){
