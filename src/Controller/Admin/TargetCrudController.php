...
class TargetCrudController extends AbstractCrudController
{
    ...
    public function validationForm($entityInstance):void
    {
        ...
        $targetBirthDate=$entityInstance->getBirthDate();

        if ($entityInstance->getCountry()!==null){
            $targetCountry = $entityInstance->getCountry()->getId();
        }
        ...
        if(!isset($targetCountry)){
            $this->session->getFlashbag('')->add('danger','Veuillez renseigner une nationalité pour la cible');
            $this->validator = false;
            //if one of the target has the same nationality of one of the agents
        }elseif ($targetBirthDate>new \DateTime('now')){
